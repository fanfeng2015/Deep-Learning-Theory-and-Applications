...
gradients = tf.gradients(mse, [theta])[0]   # compute the gradient of the mse with respect to theta
# The tf.gradients() function actually returns a list, so the [0] "unbrackets" the
# list to just return the element of the list

# create a node that assigns a new value to a variable (this implements the gradient descent algorithm)
training_op = tf.assign(theta, theta - learning_rate * gradients)   # equivalent to writing theta <-- theta - learning_rate * gradients
